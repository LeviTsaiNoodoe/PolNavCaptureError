...
//地圖授權狀態的類型
typedef NS_ENUM(NSInteger, MapPermission) {
    MapPermissionUnused = -1,   //尚未合法取得此地圖
    MapPermissionNormal = 0,    //合法取得地圖且此版本在保固期限內
    MapPermissionExpired = 1,   //合法取得地圖且此版本不在授權保固期限內
};

//地圖相關結構資料
@interface MapInfoData : NSObject
...
//地圖授權狀態
@property (nonatomic, assign) MapPermission permissionStatus;
//此版本地圖是否已安裝過授權Key
 ...
 * @param permissionStatus 地圖授權狀態
 * @param isKeyInstalled 此版本地圖是否已安裝過授權Key
 * @param isMapInstalled 此版本地圖是否已安裝在/Map/資料夾中
 * @return 生成MapInfoData地圖相關的結構資料
 */
- (instancetype)initWithMapName:(NSString *)mapName mapFileName:(NSString *)mapFileName buildDate:(NSDate *)buildDate mapCode:(NSString *)mapCode fileSize:(CGFloat)fileSize fullFileSize:(CGFloat)fullFileSize permissionStatus:(MapPermission)permissionStatus isKeyInstalled:(BOOL)isKeyInstalled isMapInstalled:(BOOL)isMapInstalled;
@end
